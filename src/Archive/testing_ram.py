...
whole_df_arr = whole_df_arr.reshape(214200, 33, 6)
...
for i in range(3,6):
    ...
    embedded_tensor = tf.keras.layers.Embedding(input_dim=layer.vocab_size()+1, input_length=(6,1), output_dim=4)(encoded_tensor)
    preprocessed_tensors.append(embedded_tensor)

combine_input = tf.keras.layers.Concatenate()(preprocessed_tensors)

lstm_1 = tf.keras.layers.Bidirectional(tf.keras.layers.LSTM(64, return_sequences=True))(combine_input)
lstm_2 = tf.keras.layers.Bidirectional(tf.keras.layers.LSTM(64, return_sequences=True))(lstm_1)
lstm_3 = tf.keras.layers.Bidirectional(tf.keras.layers.LSTM(64))(lstm_2)
dense_1 = tf.keras.layers.Dense(1)(lstm_3)

total_model = tf.keras.Model(train_input_tensor, dense_1)

total_model.compile(loss="mse", optimizer="adam", metrics="mae")

history = total_model.fit(x=train_x, y=train_y, validation_data=(val_x, val_y), epochs=100, batch_size = 200, verbose=1)
...
total_model.save('second_model', save_format='tf')
