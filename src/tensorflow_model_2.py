...
train_arr = whole_df_arr[:, :32]
val_arr = whole_df_arr[:, 1:]
...
for example in train_list:
    get_windows(example, 31, train_x, train_y)
for example in val_list:
    get_windows(example, 31, val_x, val_y)
print(val_y[0])
...
train_input_tensor = tf.keras.Input(shape=(31,7))
...
history = total_model.fit(x=train_x, y=train_y, validation_data=(val_x, val_y), batch_size=16,epochs=1, verbose=1)
...
total_model.save('../Models/Trained models/stacked LSTM large windows model', save_format='tf')

plot_model(total_model, to_file='../Models/Trained models/stacked LSTM large windows model/assets/LSTM_model.png')
